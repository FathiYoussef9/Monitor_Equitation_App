        ...
        BtnLogin.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                if (username.getText().length() > 0 && password.getText().length() > 0) {
                    JsonArrayRequest request = new JsonArrayRequest(Request.Method.GET, "https://projet-aymax.000webhostapp.com/login2.php?email=" + username.getText() + "&pass=" + password.getText(), null,
                            jsonArray -> {
                                if (jsonArray.length() > 0) {
                                    try {
                                            UserLogin.this.startActivity(myIntent);
                                            id= jsonArray.getJSONObject(0).get("clientID").toString();
